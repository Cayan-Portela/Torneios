...
for (esse in 1:length(vec)){
  dados <- getSymbols(as.character(vec[esse]),from = '2008-01-01',to = '2019-03-31')
...
for (j in 1:length(vec)){ #vec[13, 21, 43] deu erro
  ...
  YYY_3_classes_abs <- ifelse(vari_abs < -0.5, -1, ifelse(vari_abs > 0.5, +1, 0))
  YYY_3_classes_abs <- as.xts(YYY_3_classes_abs,order.by = index(close))
  
  YYY_3_classes_1desvpad_abs <- ifelse(vari_abs < -sd(vari_abs %>% na.omit()), -1, ifelse(vari_abs > sd(vari_abs %>% na.omit()), +1, 0))
  YYY_3_classes_1desvpad_abs <- as.xts(YYY_3_classes_1desvpad_abs,order.by = index(close))

  YYY_3_classes_perc <- ifelse(vari_perc < -0.01, -1, ifelse(vari_perc > 0.01, +1, 0))
  # YYY_3_classes_perc <- as.xts(YYY_3_classes_perc,order.by = index(close))
  
  YYY_3_classes_1desvpad_perc <- ifelse(vari_perc < -sd(vari_perc %>% na.omit()), -1, ifelse(vari_perc > sd(vari_perc %>% na.omit()), +1, 0))
  # YYY_3_classes_1desvpad_perc <- as.xts(YYY_3_classes_1desvpad_perc,order.by = index(close))
  ...
  BASIS <- cbind(AB_UP,AB_DOWN, AD, MFM, ADL, ADX, CHOSC, ADO, APO, AR_POS, AR_NEG, AR_OSC, ATR, ATRP, AVOL, BB_UP, BB_LOW, BB_BW, BWW,
                 ...
                 PVOI, NVOI, WILL_R, WMA, WWS, DISP, OSCP, PSY, DIU, DID, BIAS, VOLR, ARATIO, BRATIO, REX, HPR, LPR, VMOM, MPP, VARR,
                 YYY,YYY_3_classes_abs,YYY_3_classes_1desvpad_abs,YYY_3_classes_perc,YYY_3_classes_1desvpad_perc)
